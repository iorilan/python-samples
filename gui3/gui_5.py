...
from gui_mixin_3 import * # mix-in methods: quit, spawn, etc.
from gui_maker_4 import * # frame, plus menu/toolbar builder
...
class Hello(GuiMixin, GuiMakerWindowMenu): # or GuiMakerFrameMenu
    def start(self):
        ...
        def spawnme(): 
            self.spawn('gui_5.py') # defer call vs lambda
        ...
        ('Play', 0,
            ...
            ('Demos', 0,
                [('Toplevels', 0,
                lambda: self.spawn(r'..\gui2\1.py')),
                ('Frames', 0,
                lambda: self.spawn(r'..\gui2\grid_19.py')),
                ('Images', 0,
                lambda: self.spawn(r'..\gui2\img_53_thumb.py')),
                ('Alarm', 0,
                lambda: self.spawn(r'..\gui2\alarm_27.py', wait=False)),
    ...
    def fileOpen(self):
        pick = self.selectOpenFile(file='gui_5.py')
        if pick:
